...
export const editConsentRequest = async (req, res) => {
  try {
    ...
    const consent = await Consent.findOne({ _id: consentId, branchId });
    if (!consent) {
...
export const getTeacherConsents = async (req, res) => {
  try {
    ...
    const consents = await Consent.find({ classId: { $in: classIds }, branchId })
      .populate("classId", "name")
      .populate("sessionId", "name")
      .populate("createdBy", "name")
      .sort({ createdAt: -1 });
...
export const getAdminConsents = async (req, res) => {
  try {
    ...
    const query = classId ? { classId, branchId } : { branchId };
