...
export const editTimetableSlot = async (req, res) => {
  try {
    ...
    if (!timetableId || dayIndex === undefined || slotIndex === undefined || !description || !activity) {
      return res.status(400).json({ message: "Required fields missing" });
    }
    ...
    timetable.days[dayIndex].slots[slotIndex].description = description;
    timetable.days[dayIndex].slots[slotIndex].activity = activity;
    timetable.lastUpdatedBy = userId;
...
export const deleteTimetableSlot = async (req, res) => {
  try {
    ...
    if (!timetableId || dayIndex === undefined || slotIndex === undefined) {
      return res.status(400).json({ message: "Required fields missing" });
    }
    ...
    if (timetable.days[dayIndex].slots.length <= 1) {
      return res.status(400).json({ message: "Cannot delete the last slot of the day" });
    }

    timetable.days[dayIndex].slots.splice(slotIndex, 1);
    timetable.lastUpdatedBy = userId;
