...
const AddStudent = () => {
      ...
      axios
        ...
        .then((res) => {
          ...
          setFormData({
            ...
            classId: student.classId?._id || student.classId || undefined,
            name: student.name || "",
            ...
            fatherInfo: student.fatherInfo || { name: "", email: "", phoneNumber: "", occupation: "", image: undefined },
            motherInfo: student.motherInfo || { name: "", email: "", phoneNumber: "", occupation: "", image: undefined },
            ...
            transportInfo: student.transportInfo || { route: "", vehicleNumber: "", pickupPoint: "" },
            documents: student.documents?.map((doc: any) => ({
              name: doc.name,
              path: doc.path,
              file: null,
            })) || [{ name: "", file: null }],
            medicalHistory: student.medicalHistory || { condition: "good", allergies: [], medications: [] },
          });
  ...
  const removeDocumentField = (index: number) => {
    setFormData((prev) => {
      const document = prev.documents[index];
      if (document.path) {
        setDocumentsToRemove((prev) => [...prev, document.path!]);
      }
      return {
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.documents.forEach((doc, index) => {
      if (doc.name && !doc.file && !doc.path) {
        errors.push(`Document file for "${doc.name}" is required`);
      }
      if (doc.file && !doc.name) {
        errors.push(`Document name for file "${doc.file?.name}" is required`);
      }
    });
    ...
    const documentNames = formData.documents
      .filter((doc) => doc.name && (doc.file || doc.path))
      .map((doc) => doc.name);
    formDataToSend.append("documentNames", JSON.stringify(documentNames));
    if (isEdit && documentsToRemove.length > 0) {
      formDataToSend.append("documentsToRemove", JSON.stringify(documentsToRemove));
    }
    ...
    console.log("Submitting studentData:", studentData); // Debug log
                        ...
                        <div key={index} className="col-xxl col-xl-6 col-md-6 mb-3">
                          <div className="d-flex align-items-start">
                            <div className="flex-grow-1">
                              ...
                              {doc.path ? (
                                <div className="mb-2">
                                  <label className="form-label">Existing Document</label>
